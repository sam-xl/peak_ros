...
ReconstructionNodelet::ReconstructionNodelet(const rclcpp::NodeOptions & options)
  ...
  subscriber_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      "input", 100,
      std::bind(&ReconstructionNodelet::callback, this, std::placeholders::_1));
  ...
  timer_ =
      this->create_wall_timer(std::chrono::nanoseconds(1000000000 / rate_),
                              std::bind(&ReconstructionNodelet::timerCb, this));
...
ParamType ReconstructionNodelet::paramHandler(std::string param_name,
...
void ReconstructionNodelet::initialisePointcloud() {
  ...
  modifier.setPointCloud2Fields(
      fields, "x", 1,
      sensor_msgs::msg::PointField::FLOAT32, // 32 bits = 4 bytes
      "y", 1, sensor_msgs::msg::PointField::FLOAT32, "z", 1,
...
void ReconstructionNodelet::callback(
...
bool ReconstructionNodelet::publishSrvCb(
  ...
  RCLCPP_INFO_STREAM(this->get_logger(),
                     node_name_ << ": Publish UT volume request received: "
                                << request->stream_data);

  if (request->stream_data) {
    publisher_->publish(point_cloud_);
    RCLCPP_INFO_STREAM(this->get_logger(),
                       node_name_ << ": Published reconstruction");
...
void ReconstructionNodelet::timerCb() {
  ...
  if (!buffer_.empty()) {
    ...
    if (use_tf_) {
      try {
        trans_ = tfBuffer_->lookupTransform(
            recon_frame_id_,                          // target frame
        ...
        tf2::doTransform<sensor_msgs::msg::PointCloud2>(
            *msg, output_pointcloud2, trans_);

        point_cloud_.width += output_pointcloud2.width;
        uint64_t prev_size = point_cloud_.data.size();
        point_cloud_.data.resize(point_cloud_.data.size() +
                                 output_pointcloud2.data.size());

        std::copy(output_pointcloud2.data.begin(),
        ...
        RCLCPP_WARN_STREAM(
            this->get_logger(),
            node_name_ << ": Could not find transform " << recon_frame_id_
                       << " to " << msg->header.frame_id << ": " << ex.what());
      }
      ...
      if (b_scan_count_ == 0) {
        ...
        rclcpp::Duration dt =
            rclcpp::Time(msg->header.stamp) - prev_observation_time_;
        ...
        if (dt.seconds() < 4.0l) {
          ...
          if (flip_direction_) {
            ...
            RCLCPP_INFO_STREAM(this->get_logger(),
                               node_name_ << ": Reset start of pass to zero: "
                                          << trans_.transform.translation.x);
          }
        }
      }

      tf2::doTransform<sensor_msgs::msg::PointCloud2>(*msg, output_pointcloud2,
                                                      trans_);

      // RCLCPP_INFO_STREAM_THROTTLE(this->get_logger(), *this->get_clock(),
      // 10000, node_name_ << ": transform translation x: " <<
      // trans_.transform.translation.x);
      // RCLCPP_INFO_STREAM_THROTTLE(this->get_logger(), *this->get_clock(),
      // 10000, node_name_ << ": transform translation y: " <<
      // trans_.transform.translation.y);
      // RCLCPP_INFO_STREAM_THROTTLE(this->get_logger(), *this->get_clock(),
      // 10000, node_name_ << ": transform translation z: " <<
      ...
      std::copy(output_pointcloud2.data.begin(), output_pointcloud2.data.end(),
                point_cloud_.data.begin() + prev_size);
